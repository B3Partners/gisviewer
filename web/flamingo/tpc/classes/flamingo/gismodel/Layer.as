...
class flamingo.gismodel.Layer extends AbstractComposite implements ActionEventListener {
    ...
    function addFeature(_object:Object):Void {
        if (_object == null) {
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.addFeature()\No service feature or geometry given.");
            return;
        }
        if (!(_object instanceof ServiceFeature) && !(_object instanceof Geometry)) {
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.addFeature()\Given object is not a service feature or a geometry.");
            return;
        }
    ...
    function removeFeatures(features:Array, addOperation:Boolean):Void {
        if (features == null) {
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.removeFeatures()");
            return;
        }
    ...
    function removeFeature(feature:Feature, addOperation:Boolean):Void {
        if (feature == null) {
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.removeFeature()");
            return;
        }
    ...
    private function doRemoveFeature(feature:Feature, addOperation:Boolean):Void { // Removes the given feature, without dispatching an event.
        ...
        if (featurePosition == -1) {
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.doRemoveFeature()");
            return;
        }
    ...
    function addEventListener(stateEventListener:StateEventListener, sourceClassName:String, actionType:Number, propertyName:String):Void {
        if (
            ...
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.addEventListener(" + sourceClassName + ", " + propertyName + ")");
            return;
        }
    ...
    function removeEventListener(stateEventListener:StateEventListener, sourceClassName:String, actionType:Number, propertyName:String):Void {
        if (
            ...
            _global.flamingo.tracer("Exception in flamingo.gismodel.Layer.removeEventListener(" + sourceClassName + ", " + propertyName + ")");
            return;
        }
