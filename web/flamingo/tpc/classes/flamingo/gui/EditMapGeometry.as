...
class flamingo.gui.EditMapGeometry extends GeometryPane implements StateEventListener {
    ...
    function onLoad():Void {
        ...
        gis.addEventListener(this, "GIS", StateEvent.CHANGE, "extent");
    ...
    function remove():Void { // This method is an alternative to the default MovieClip.removeMovieClip. Also unsubscribes as event listener. The event method MovieClip.onUnload cannot be used, because it works buggy.
        gis.removeEventListener(this, "GIS", StateEvent.CHANGE, "extent");
    ...
    function onStateEvent(stateEvent:StateEvent):Void {
        ...
        if (sourceClassName + "_" + actionType + "_" + propertyName == "GIS_" + StateEvent.CHANGE + "_extent") {
            draw();
        } else if (sourceClassName + "_" + actionType + "_" + propertyName == "Geometry_" + StateEvent.CHANGE + "_null") {
            draw();
    ...
    private function point2Pixel(_point:flamingo.geometrymodel.Point):Pixel {
        var extent:flamingo.geometrymodel.Envelope = gis.getExtent();
        var minX:Number = extent.getMinX();
        var minY:Number = extent.getMinY();
        var maxX:Number = extent.getMaxX();
        var maxY:Number = extent.getMaxY();
